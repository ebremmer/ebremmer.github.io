...
;( function( window ) {
	...
	stepsForm.prototype._validade = function() {
		...
		switch( inputEl.id ) {
			...
			case 'mail' :
				...
				break;
			case 'dataagreement' :
				var filter=/^([T|t][A|a]?[K|k]?)$/gi;
				if (filter.test(input))
					return true;
				else{
					this._showError( 'NOPERM' );
					return false;
				}
				break;
			case 'partneragreement' :
				var filter=/^([T|t][A|a]?[K|k]?)|([N|n][I|i]?[E|e]?)$/gi;
				if (filter.test(input))
					return true;
				else{
	...
	stepsForm.prototype._showError = function( err ) {
		...
		switch( err ) {
			case 'EMPTYSTR' :
				message = 'Prosimy o poprawną odpowiedź przed kontynuacją';
				break;
			case 'INVALIDEMAIL' :
				message = 'Proszę podać poprawny e-mail';
				break;
			case 'INVALIDPHONE' :
