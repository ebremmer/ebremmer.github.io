...
(function(window) {
        ...
        success: function(res) {
          var nlocation =
            ...
            "&z24_nazwa=" + encodeURIComponent(res.id +
              "-orthoday") + "&z24_kwota=" + encodeURIComponent((
  ...
  FForm.prototype._nextField = function(backto) {
    ...
    this.isLastStep = this.current === this.fieldsCount - 1 && backto ===
    ...
    this.navdir = backto !== undefined ? backto < this.current ? 'prev' :
      'next' : 'next';
    ...
    if (support.animations) {
      if (this.navdir === 'next') {
        if (this.isLastStep) {
          currentFld.querySelector('.fs-anim-upper').addEventListener(
            animEndEventName, onEndAnimationFn);
        } else {
          nextField.querySelector('.fs-anim-lower').addEventListener(
            animEndEventName, onEndAnimationFn);
        }
      } else {
        nextField.querySelector('.fs-anim-upper').addEventListener(
          animEndEventName, onEndAnimationFn);
      }
    } else {
      onEndAnimationFn();
